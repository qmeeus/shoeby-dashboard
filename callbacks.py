...
def update_production_text(inventory, brands, categories, month_slider):
    ...
    abs_gap_summary = matenboog.groupby(level=0).agg(lambda s: abs(s).sum())
    absolute = "Absolute gap: {:02.2f}% ".format(abs_gap_summary['gap'].sum() * 100)
    return absolute
